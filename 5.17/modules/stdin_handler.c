...
static int is_words_divider(char c)
{
    if (c == ' ' || c == '\t')
        return 1;
...
static int is_cmdline_divider(char c)
{
    if (c == '&' || c == '|' || c == '<' || c == '>' || c == '(' || c == ')' || c == ';')
        return 1;
...
static int are_cmdline_divider_parts(char c, char c2)
{
    if (c == '&' && c2 == '&')
        return 1;
...
void read_from_stdin(struct item **list, void (*callback)(struct item *))
{
    ...
    {        
        ...
        if (!are_dividers_ignored && is_words_divider(c)) {
            if (char_adding_mode == new_word_mode && prev_char == '"' && prev_prev_char == '"' && (prev_prev_prev_char == ' ' || prev_prev_prev_char == initial_char_value)) {
                add_item_to_list(list, '\0', new_word_mode);
            }
            char_adding_mode = new_word_mode;
        } else if (!are_dividers_ignored && is_cmdline_divider(c)) {
            if (are_cmdline_divider_parts(c, prev_char)) {
                add_item_to_list(list, c, new_char_mode); 
            } else {
                add_item_to_list(list, c, new_word_mode);
            }
            char_adding_mode = new_word_mode;
        } else if (c == '"' && prev_char != '\\') {
